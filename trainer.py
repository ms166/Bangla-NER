...
def train(model, iterator, optimizer, criterion, epoch, focal_loss_fn):
    model.train()
    for i, batch in enumerate(iterator):
        ...
        assert(not torch.isnan(logits_focal).any())

        logits_focal = logits_focal.view(-1, logits_focal.shape[-1])
        y_focal = y.view(-1)

        cce_loss = criterion(logits_focal, y_focal)
        focal_loss = focal_loss_fn(logits_focal, y_focal)
        loss = focal_loss + cce_loss
        loss.backward()
...
def eval(model, iterator, epoch=None):
    ...
    if epoch is not None:
        str_report = classification_report(y_true,y_pred, labels = [1,2,3,4,5,6,7,8,9], digits=4, zero_division=0)
        dict_report = classification_report(y_true,y_pred, labels = [1,2,3,4,5,6,7,8,9], digits=4, zero_division=0, output_dict=True)
        print(f"============Evaluation at Epoch={epoch}============")
        print(str_report)
    os.remove("results.txt")

    return all_preds, dict_report
